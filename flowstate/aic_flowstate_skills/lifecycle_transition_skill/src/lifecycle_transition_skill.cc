...
// Assuming INTR_ASSIGN_OR_RETURN is available via some included header or
// transitively. If not, we might need to include
// "intrinsic/util/status/status_macros.h" if it exists.
...
LifecycleTransitionSkill::Execute(
  ...
  auto client =
      node->create_client<lifecycle_msgs::srv::ChangeState>(service_name);
  ...
  if (!response->success) {
    return absl::InternalError("Lifecycle transition failed for node " +
                               node_name);
  }

  RCLCPP_INFO(node->get_logger(), "Successfully transitioned node %s",
              node_name.c_str());
