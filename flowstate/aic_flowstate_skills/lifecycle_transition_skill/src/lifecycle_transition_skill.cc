...
class LifecycleClientNode : public rclcpp::Node {
 public:
  ...
  absl::StatusOr<bool> ChangeState(const std::string& node_name, uint8_t transition, double timeout_ms) {
...
LifecycleTransitionSkill::Execute(
  ...
  RCLCPP_INFO(client_node_.get_logger(), "Transitioning node %s to transition %d",
              params.node_name().c_str(), params.transition());

  uint8_t transition_id = static_cast<uint8_t>(params.transition());

  auto status_or_success = client_node_.ChangeState(params.node_name(), transition_id, 10000.0);

  if (!status_or_success.ok()) {
    return status_or_success.status();
  }

  auto result = std::make_unique<ai::flowstate::LifecycleTransitionSkillResult>();
