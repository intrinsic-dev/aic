...
src/aic/flowstate/scripts/build_container.sh \
  ...
  --service_name aic_logger \
  --service_package aic_logger \
  --dependencies "ros-kilted-rosbag2,ros-kilted-rosbag2-storage-mcap" \
  --dockerfile src/aic/flowstate/resources/Dockerfile.service

echo "INFO: Loading newly built service image into local daemon..."
docker load -i "images/aic_logger/aic_logger.tar"

echo "INFO: Extracting config and descriptor set from container..."
docker create --name temp_container_service aic_logger:aic_logger
docker cp "temp_container_service:/opt/ros/overlay/install/share/aic_logger/aic_logger_protos.desc" \
  "images/aic_logger/aic_logger_protos.desc"
docker rm -f temp_container_service

echo "INFO: Building the service bundle..."
./inbuild service bundle \
  --file_descriptor_set images/aic_logger/aic_logger_protos.desc \
  --manifest src/aic/flowstate/aic_logger/aic_logger.manifest.textproto \
  --oci_image images/aic_logger/aic_logger.tar \
  --output images/aic_logger/aic_logger.bundle.tar \
  --default_config src/aic/flowstate/aic_logger/aic_logger_default_config.pbtxt
