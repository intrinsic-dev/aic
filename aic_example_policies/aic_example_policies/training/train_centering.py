"""Train the Phase 1 (centering) SAC policy.

Usage:
    python -m aic_example_policies.training.train_centering \\
...
Notes:
- Ensure your scene.xml has correct body/site names (see CenteringConfig).
- The detection auxiliary loss needs ground truth (u,v) from the MuJoCo camera.
  This is computed each step via project_point_to_image().
- To disable the detection loss: set --det_loss_weight 0
"""
...
from .centering_env import CenteringConfig, CenteringEnv
from .image_utils import project_point_to_image, normalize_uv, IMG_SIZE
from .networks import P1_OBS_DIM
...
def main():
    ...
    print(f"Training Phase 1 on {DEVICE}. Total steps: {args.total_steps}")
    ...
    while total_steps < args.total_steps:
        ...
        # Compute GT detection label for this transition
