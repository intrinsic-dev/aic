...
- The `aic_model` node must exhibit the following behavior
  - Start in `unconfigured` lifecycle state.
	  - In `unconfigured` state, no topics should be published by node, espcially to command the robot.
	- A transition to `configred` state must succeed within a timeout of 60s. Loading of any models should occur within this window.
	  - In `configured` state, no topics should be published by node, espcially to command the robot.
	...
	- A `shutdown` transition request must succeed within a timeout of 60s.
		- In `shutdown` state, no topics should be published by node, espcially to command the robot.
