...
| `/expand_xacro` | `aic_engine_interfaces/srv/ExpandXacro` | Expand a xacro file from an installed ROS package into XML. The request specifies a `package_name`, a `relative_path` within that package's share directory, and optional `xacro_arguments`. Returns the expanded XML string. Useful for programmatic entity spawning during training (e.g. task boards, cables) without requiring direct filesystem access to the eval environment. |
